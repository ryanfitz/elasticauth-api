...
internals.find = function (log, params, options, callback) {
  ...
  Account.get(params.id, opts, function (err, acc) {
    if(err) {
      log.error({err : err}, 'Failed to find accounts');
      return callback(err);
    } else if (_.isNull(acc)) {
      return callback(null, []);
    } else {
      return callback(null, [acc.get()]);
    }
  });
};
