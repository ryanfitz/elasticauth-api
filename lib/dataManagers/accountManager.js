...
internals.convertFieldsStringToProjectionExpression = function(fieldsString, allowAllFields) {
  var whiteList = ['id', 'username', 'name', 'metadata'];

  var fields = _.filter(_.words(fieldsString), function (word) {
    if(allowAllFields === true) {
      return true;
    } else {
      return _.includes(whiteList, word);
    }
  });

  var res = _.reduce(fields, function (result, key) {
    var attributeKey = '#' + key;
    result.keys.push(attributeKey);
    result.names[attributeKey] = key;

    return result;
  }, {keys : ['#id'], names: {'#id' : 'id'} });

  return { ProjectionExpression : _.uniq(res.keys).join(','), ExpressionAttributeNames : res.names};
};

internals.accountProjections = function (allFields, fieldsToGet) {
  if(!_.isEmpty(fieldsToGet)) {
    return internals.convertFieldsStringToProjectionExpression(fieldsToGet, allFields);
  } else if(allFields) {
    return {};
  } else {
    return {
...
internals.find = function (log, params, options, callback) {
  ...
  var opts = _.merge({}, internals.accountProjections(options.allFields, options.fieldsToGet));
