...
internals.createCredential = function (type, authId, accountId, callback) {
  if(authId) {
    var key = (type + ':' + authId).toLowerCase();

    var params = {};
    params.ConditionExpression = '(#i <> :k) OR (#i = :k and #acc = :aid)';
    params.ExpressionAttributeNames = {'#i' : 'id', '#acc' : 'accountId'};
    params.ExpressionAttributeValues = {':k' : key, ':aid' : accountId};
...
internals.removeCredential = function (type, authId, accountId, callback) {
  ...
  if(authId) {
    ...
    var params = {};
    params.ConditionExpression = '#a = :accid OR attribute_not_exists(#i)';
    params.ExpressionAttributeNames = {'#a' : 'accountId', '#i' : 'id'};
...
module.exports = function (logger) {
  return {
    ...
    createCredential : internals.createCredential,
    removeCredential : internals.removeCredential,
  };
};
